define(function(require){
	...
	//This script makes window.DOM available.
	var betterDOM = require('dashboard/vendor/shim/better-dom');

var initBetterDateInput = function(DOM, COMPONENT_CLASS, I18N_DAYS, I18N_MONTHS) {
    "use strict";
    ...
    DOM.extend("input[type=date]", !("orientation" in window), {
        constructor: function() {
            var calendar = DOM.create("div.{0}>a[unselectable=on]*2+span.{0}-header+table.{0}-days>thead>tr>th[unselectable=on]*7+tbody>tr*6>td*7", [COMPONENT_CLASS + "-calendar"]),
                ...
                offset = this.offset(),
                calOffset;
            //ADDED by NJ 2 team to support defaulting the value
            //Line 30 value:theInitialValue was value:Date.now()
            var theInitialVal = this.get("defaultValue"); //seems to be populated from input value

            if(!theInitialVal||(theInitialVal&&theInitialVal=="Today")) {
            	theInitialVal = Date.now();
            }
            else if(theInitialVal) {
            	theInitialVal = new Date(theInitialVal);
            }
            //END ADD
            this
                // remove legacy dateinput implementation if it exists
                // also set value to current time to trigger watchers later
                .set({type: "text", value: theInitialVal})
        ...
        onValueChanged: function(displayedValue, caption, weekdays, days, value) {
            var year, month, date, iterDate, parts = value.split('-');
            displayedValue.set("");

            //EDITED by NJ 2 team to support defaulting the value
            var isShowToday = value == "Today";
            value = value=="Today" ? new Date() : new Date(parts[0], parts[1]-1, parts[2]);
            //END EDIT
            ...
            if (value.getTime()) {
                ...
                displayedValue
                	...
                	if(isShowToday ) {
                		xDisplayedValue
                		.append(DOM.create("span").append("&nbsp;" + "Today"));
                	}
                	else {
                		xDisplayedValue
                	//END
...
//Don't need below setTimeout logic if we have
//line 3 var betterDOM = require('dashboard/vendor/shim/better-dom');
//because better-dom actually defines window.DOM
(initBetterDateInput(window.DOM, "better-dateinput", [
...
var timePassed = 0;
var SETTIMEOUT_TIME = 100;

var logC = function(msg) {
...
//Would prefer to create a require module that depends on window.DOM, so this following approach
// is not ideal
//but this plugin depends on window.DOM which seems to be a browser-generated variable, not from
//a require module.

var initBetterDateInputOnDOMLoad = function() {

	if(window.DOM) {

		logC("initBetterDateInput: found window.DOM. Running...");
	...
	else if(timePassed <= MAX_TIME) {
		logC("initBetterDateInput: initBetterDateInput is delayed by " +
				SETTIMEOUT_TIME +
			 " because window.DOM was found as undefined still");

		setTimeout(initBetterDateInputOnDOMLoad, SETTIMEOUT_TIME);
		timePassed = timePassed + SETTIMEOUT_TIME;
	}
};

initBetterDateInputOnDOMLoad();
