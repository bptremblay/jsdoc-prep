define(function (require) { // TODO: component needs to haave support for il8n
	var componentChannel = require('blue/event/channel/component'),
		Utils = require('dashboard/component/myProfile/utils'),
		context = null,
		self = null,
	...
	return {
		init: function() {
			...
			model.selectedDate = model.inputValue ? (new Date(model.inputValue)) : (new Date());
		...
		selectDate: function($target) {
			var selectedDate = new Date($target.data("date"));
			$('#' + this.model.get().inputId).val($target.data("date")).focus();
			this.model.lens('selectedDate').set(selectedDate);
			this.hide();
		},
