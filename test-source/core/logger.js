  ...
  DefaultLogger.prototype.log = function(msg, level) {
    ...
    if (LOG_TO_SERVER) {
      ...
      if (msg.indexOf('PROFILER REPORT') != -1) {
        $.get(YUI_config.app.store_url
          + '/sandbox/dev-logger/dev-logger.php?msg=' + escape(msg));
      } else {
        $.get(YUI_config.app.store_url
          + '/sandbox/dev-logger/dev-logger.php?msg='
          + escape(level + ': ' + msg));
      }
    }
      ...
      if (instance === null) {
        ...
        if (LOG_TO_SERVER) {
          window.setTimeout(function() {
            $.get(YUI_config.app.store_url
              + '/sandbox/dev-logger/dev-logger.php?cmd=reset');
          }, 1);
        }
      }
