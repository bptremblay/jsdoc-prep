...
define( function( require, exports, module ){
	var compose		 = require( './compose' ),
		...
		is 			 = require( './is' ),
		sessionStore = new ( require( './store/enumerable/session' ) )( 'servicesettings' );

		config 		= module.config(),
		services	= config.services;

	return require( './declare' )( function(){
		var servicePrototype = /** @lends module:blue/service# */ {
				 ...
				 * @augments {null}
				 * @param {String} this.serviceCalls - defined in derived classes
				 *
				 ...
				 *		config: {
				 *			'service' : {
				 *				primaryUrl : 'http://localhost:9000/bluejs-mvc/',
				 *				backupUrl : 'http://localhost:9000/bluejs-mvc/'
				 *			},
				 *			serviceMappings : {
				 *				//readDetail : 'rl/readDetail/list',
				 *				readDetail: 'data/detail.json'
				 *			}
				 *		}
				 *	};
				 * </script>
				...
				constructor: function Service(){
					...
					is.plainObject( descriptor ) && Object.keys( descriptor ).forEach( function( key ){
						var settings = extend( true, {}, defaults, this.serviceCalls[ key ].settings );

						//for now we are only going to use the primaryURL, retry logic is not currently in scope
						services.forEach( function( service ){
							if( service.name === key ){
								if( is.defined( service.urls[ 0 ] ) ){
					...
					this.enableInterceptors( this.interceptors || [] );
				...
				prerequest: function( settings, data ) {
					...
					settings.data = data || {};
					if (settings.url.substr(0,4).toLowerCase().indexOf( 'http' ) !== 0) {
		...
		compose( servicePrototype, function(){ require( './intercept' ).withInterceptors.call( this, 'request' ); } );
