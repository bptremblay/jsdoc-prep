define([],
		...
		function() {
			...
			var withExponents = {
				...
				// };
				/**
				 * Takes any number of Number pairs as arguments, each one being
				 * a 2-element Array.
				 * 
				...
				multiplyExp : function() {
					...
					for (var index = 0; index < arguments.length; index++) {
						...
						if (typeof pair === 'object' && pair.length === 2) {
							...
							throw (new Error(
									'withExponents.multiplyExp() failed with invalid arguments: '
											+ String(pair)));
						}
					}
				...
				divideExp : function() {
					...
					for (var index = 0; index < arguments.length; index++) {
						...
						if (typeof pair === 'object' && pair.length === 2) {
							...
							// result.
							if (index > 0) {
								previousPowerOfPair = previousPowerOfPair
										/ powerOfPair;
							} else {
								previousPowerOfPair = powerOfPair;
							}
							...
							throw (new Error(
									'withExponents.divideExp() failed with invalid arguments: '
											+ String(pair)));
						}
					}
					return previousPowerOfPair;
			...
			exports.withExponents = withExponents;
			return exports;
		});