...
var fixDecaffeinateProc = {
  ...
  process: function (input, doneCallback) {
    var useInitClass = false;
    logger.log('****************** fixDecaffeinateProc *******************');
    var source = input.source;
    var grunt = input.grunt ? input.grunt : null;
    ...
    var beforeDefine = source.split('define(')[0].trim();
    if (beforeDefine.indexOf('Removed the __initClass__()') !== -1) {
      beforeDefine = beforeDefine.split('Removed the __initClass__()').join('');
      useInitClass = true;
    }
    var stripped = beforeDefine.split('//').join('');
    ...
    stripped = stripped.split('  ').join(' ');
    stripped = stripped.split('  ').join(' ');
    beforeDefine = stripped.split(' * ').join('');
    beforeDefine = beforeDefine.split('.').join('.<br>\n * ');
    beforeDefine = beforeDefine.trim();
    // beforeDefine = stripped.split('\n').join(' ');
    //console.log("beforeDefine >>>>>>>>>>>>>>>>>>> ", beforeDefine);
    //        if (source.indexOf('export default class') !== -1) {
    //            // This macro is injected by decaffeinate on the same line as the class declaration.
    //            var classNameGuess = source.split('export default class ')[1].split(' ')[0].trim();
    //            source = source.split('export default class ').join('\n/** ES2015 Class Declaration */\nclass ');
    //            source = source + '\nexport default ' + classNameGuess + ';\n';
    //        }
    var importPaths = [];
      ...
      if (someNode.expression.callee && someNode.expression.callee.name === 'define') {
        ...
        if (oneDefine.expression.arguments.length === 0) {
          ...
          if (!imports) {
            imports = [];
            importedAs = [];
            //var err = new Error('fixDecaffeinateProc failed...');
            //logger.error('fixDecaffeinateProc failed...', err.stack);
            //continue;
          }
    ...
    if (oneDefine) {
      ...
      if (exportReturnedNode.type === 'ArrowFunctionExpression') {
        ...
        node = node.body;
        var codeBody = null;
        // don't omit the decaffeinated helper functions which MAY BE AT THE BOTTOM of module
        var codeFooter = null;
        if (node.type === 'ObjectExpression') {
          range = node.range;
          exportReturned = input.source.substring(range[0], range[1]).trim();
          codeFooter = input.source.substring(range[1]).trim();
        ...
        var whereDoclet = codeFooter.indexOf('/**');
        if (whereDoclet !== -1) {
          codeFooter = codeFooter.substring(whereDoclet);
        }
        if (codeFooter.trim().indexOf('}') === 0 || codeFooter.trim().indexOf(')') === 0 || codeFooter.trim().indexOf(';') === 0) {
          codeFooter = '';
        } else {
          logger.log('%%%%%%%%%%%%%% FOOTER');
          logger.log(codeFooter);
        }
        ...
        if (exportedValue && exportedValue.root) {
          ...
          logger.log('codeBody A');
        } else if (node.type === 'CallExpression') {
          codeBody = exportReturned + ';';
          logger.log('codeBody B');
        } else if (node.type === 'ReturnStatement') {
          codeBody = codeBody.split(exactReturn).join('export default ' + exportReturned + ';');
          logger.log('codeBody C');
        } else if (exportReturned.trim().indexOf('class ') === 0) {
          codeBody = 'export default ' + exportReturned;
          logger.log('codeBody D');
        } else {
          codeBody = 'export default ' + exportReturned + ';';
          logger.log('codeBody E');
        }
        var codeHeader = '';
        codeHeader += '/**\n';
        codeHeader += ' * ' + beforeDefine + '\n';
        ...
        codeHeader += ' */\n\n';
        //FIXME include babel polyfill if necessary
        //        if (isSpec) {
        //          var polyFillSrc = readFile('./tasks/lib/babel-polyfill-min.js');
        //          if (!polyFillSrc) {
        //            throw (new Error('Can not find the poly.'));
        //          }
        //          var splitPath = input.path.split('/');
        //          splitPath.pop();
        //          var polyPath = splitPath.join('/');
        //          console.log('polyPath', polyPath);
        //          writeFile(polyPath + '/browser-polyfill.js', polyFillSrc);
        //          importPaths.push({
    ...
    input.source = input.source.split('export default undefined').join('// export default undefined');
    if (input.source.indexOf('export default class ') !== -1) {
      // This macro is injected by decaffeinate on the same line as the class declaration.
      var classNameGuess = input.source.split('export default class ')[1].split(' ')[0].trim();
      input.source = input.source.split('export default class ').join('\n/** The class ' + classNameGuess + '. */\nclass ');
      if (useInitClass) {
        input.source = input.source + '\n ' + classNameGuess + '.initClass();\n';
      }
      input.source = input.source + '\nexport default ' + classNameGuess + ';\n';
    }
    // if (input.source.indexOf(beforeDefine) === -1) {
    //input.source = beforeDefine + '\n\n' + input.source;
    // }
    //        if (input.source.indexOf('export default class ') === -1 && input.source.indexOf('class ') !== -1) {
    //            // This macro is injected by decaffeinate on the same line as the class declaration.
    //            var classNameGuess = input.source.split('class ')[1].split(' ')[0].trim();
    //            input.source = input.source + '\nexport default ' + classNameGuess + ';\n';
    //        }
    // if (input.source.indexOf('Array.from') !== -1){
    //         	logger.warn('******** ', amdProcData.moduleName, ': found Array.from');
    //         	var arrayFromSource = arrayFrom.getSource();
    //         	//console.log('arrayFrom.length = ', arrayFromSource.length);
    //         	input.source = input.source + '\n//////////////////////// POLYFILL //////////////////////\n' + arrayFromSource + '\n';
    //         }
    //
    //         if (input.source.indexOf('Symbol.iterator') !== -1){
    //         	logger.warn('******** ', amdProcData.moduleName, ': found Symbol.iterator');
    //         	var symbolIteratorSource = symbolIterator.getSource();
    //         	//console.log('arrayFrom.length = ', arrayFromSource.length);
    //         	input.source = input.source + '\n//////////////////////// POLYFILL //////////////////////\n' + symbolIteratorSource + '\n';
    //         }
    if (isSpec) {
      var polyFillSrc = readFile('./tasks/lib/babel-polyfill-min.js');
      if (!polyFillSrc) {
...
function buildES6Header(input) {
  ...
  for (es = 0; es < importStatements.length; es++) {
    ...
    //try{
     var symbol = '';
    if (someNode.specifiers.length){
    symbol = someNode.specifiers[0].local.name;
    }
    ...
    // }
//     catch(exxx){
//       console.error(exxx.stack, someNode.specifiers);
//     }
  }
...
var getES6ModuleDataProc = {
  id: 'getES6ModuleDataProc',
  type: 'processor',
  description: 'gets imports/exports.',
  process: function (input, doneCallback) {
    logger.log('****************** fixES6ModulesProc *******************');
    var source = input.source;
    var amdProcData = input.results.amdProc;
    var isAMD = (source.indexOf('define (') !== -1 || source.indexOf('define(') !== -1);
    if (isAMD) {
      logger.log('getES6ModuleDataProc BAILING; this file is AMD.');
      doneCallback(input);
      return;
    }
    var header = buildES6Header(input);
...
// TODO: make it work
function createTestSpec(input, amdProcData, classData, doneCB) {
  var index = 0;
  ...
  if (classData.jsDoccerProcData) {
    ...
    console.error("ERROR: no createTestSpec");
  }
  //  package com.ctct.galileoUsers.btremblay.src.galileo-document-editor-lib.src;
  //public class DocumentRendererJsDecaf {
  var projectRoot = input.projectRoot;
  var exportPath = 'specs-generated';
  // console.log('camel?', input.camelName);
  ...
  // console.log('camel?', input.camelName);
  if (input.camelName.indexOf('.') !== -1) {
    input.camelName = input.camelName.split('.')[0];
    input.camelName = camelize(input.camelName);
  }
  // console.log('camel?', input.camelName);
  //console.log('projectRoot PATH???',  input.projectRoot);
  //eded();
  var packageSubpath = AMD ? amdProcData.webPath : input.packagePath;
  //console.log('>>>>>>>>>>>>>>> ?????????????????? packageSubpath: ', packageSubpath, projectRoot);
  if (projectRoot.indexOf('/') !== -1) {
    packageSubpath = packageSubpath.substring(projectRoot.length);
  }
  input.packageSubpath = packageSubpath;
  var deps = [];
  if (AMD) {
  ...
  if (input.importPaths) {
    //[ { path: 'underscore', name: '_' } ]
    // console.log(input.importPaths);
    // sikslks();
    deps = [];
    for (var x = 0; x < input.importPaths.length; x++) {
      var p = input.importPaths[x];
      deps.push(p.path);
    }
  }
  ...
  //  });
  //});
  //import Editor from 'action-block-editor-path/index';
  var modulePath = input.path;
  modulePath = modulePath.split('.js')[0];
  var len = projectRoot.length;
  modulePath = modulePath.substring(len);
  if (modulePath.charAt(0) === '/') {
    modulePath = modulePath.substring(1);
  }
  modulePath = '../' + modulePath;
  var realFileName = modulePath.split('/').pop();
  var classFileName = realFileName + '-spec.js';
  buffer.push("import " + capitalize(input.camelName) + " from '" + modulePath + "';");
  buffer.push('');
  buffer.push('beforeEach(() => {');
  buffer.push('});');
  buffer.push('afterEach(() => {');
  buffer.push('});');
  buffer.push('');
  buffer.push('describe("' + capitalize(input.camelName) + '", function() {');
  if (deps == null) {
    deps = [];
  }
  var members = [];
  for (index = 0; index < deps.length; index++) {
    ...
    // buffer.push('  import com.ctct.galileo.' + capitalize(camelName) + ';');
    members.push('  public ' + capitalize(camelName) + ' ' + memberName + ' = null;');
  }
  //buffer.push('public class ' + capitalize(input.camelName) + ' {');
  //buffer.push(members.join('\n'));
  //buffer = buffer.concat(members);
  for (index = 0; index < methods.length; index++) {
    ...
    if (method.name.indexOf("_") === 0) {
      ...
      buffer.push('describe("' + (method.name) + '", function() {');
      buffer.push('  it("' + 'Does something' + '", function() {');
      buffer.push('    expect(true).toBe(true);');
      buffer.push('  });');
      buffer.push('});');
    }
  }
  ...
  buffer.push('});');
  var src = buffer.join('\n');
  var specPath = exportPath + '/';
  if (packageSubpath) {
    if (packageSubpath.indexOf('/') === 0) {
      packageSubpath = packageSubpath.substring(1);
    }
    if (packageSubpath.indexOf('/') === (packageSubpath.length - 1)) {
      packageSubpath = packageSubpath.substring(0, packageSubpath.length - 1);
    }
    specPath += packageSubpath + '/';
  }
  specPath += classFileName;
  //console.log('WRITE SPEC TO', specPath);
  writeFile(specPath, src);
  var saveSource = input.source;
  var savePath = input.processedFilePath;
  input.source = src;
  input.processedFilePath = specPath;
  jsBeautifyProc.process(input, function () {
    input.source = saveSource;
    input.processedFilePath = savePath;
    doneCB();
  });
}
/**
 ...
 * @param directory
 */
function formatJava(directory) {
  var exePath = 'java -jar google-java-format-1.3-all-deps.jar';
  var exec = require('child_process').exec;
  var child = exec(exePath + ' --replace ' + directory, function (error, stdout, stderr) {
    if (error) {
      console.error(error);
    }
    if (stdout) {
...
function createJavaClass(input, amdProcData, classData) {
  ...
  //console.log('PACKAGE PATH???',  amdProcData.webPath, input.packagePath);
  //eded();
  var packageSubpath = AMD ? amdProcData.webPath : input.packagePath;
  var projectRoot = input.projectRoot;
  //packageSubpath = packageSubpath.substring(projectRoot.length);
  ...
  if (subPackage.indexOf('.') === (subPackage.length - 1)) {
    subPackage = subPackage.substring(0, subPackage.length - 1);
  }
  subPackage = subPackage.split('-').join('');
  subPackage = subPackage.split('_').join('');
  var totalPackage = '';
  if (subPackage.trim().length) {
    buffer.push('package com.ctct.galileo.ui.' + subPackage + ';');
    totalPackage = 'com.ctct.galileo.ui.' + subPackage;
  } else {
    buffer.push('package com.ctct.galileo.ui;');
    totalPackage = 'com.ctct.galileo.ui';
  }
  if (deps == null) {
    deps = [];
  }
  ...
  for (index = 0; index < deps.length; index++) {
    ...
    importPath = importPath.split('-').join('');
    importPath = importPath.split('_').join('');
    importPath = importPath.split('/');
    importPath.pop();
    importPath = importPath.join('/');
    importPath = importPath.split('../').join('');
    importPath = importPath.split('./').join('');
    // remove any dots before using dots as a package delim
    importPath = importPath.split('.').join('');
    // swap delims
    importPath = importPath.split('/').join('.');
    if (importPath.indexOf('.') === 0) {
    ...
    memberName[0] = memberName[0].toLowerCase();
    memberName = memberName.join('');
    var classBranch = importPath;
    if (importPath.charAt(importPath.length - 1) !== '.') {
      classBranch += '.';
    }
    if (classBranch === '.') {
      classBranch = '';
    }
    buffer.push('  import com.ctct.galileo.' + classBranch + capitalize(camelName) + ';');
    members.push('  public ' + capitalize(camelName) + ' ' + memberName + ' = null;');
  }
  if (input.classes[capitalize(input.camelName)]) {
    extendsClass = input.classes[capitalize(input.camelName)].extends;
  }
  if (extendsClass) {
    buffer.push('public class ' + capitalize(input.camelName) + ' extends ' + extendsClass + ' {');
  } else {
    buffer.push('public class ' + capitalize(input.camelName) + ' {');
  }
  totalPackage += '.' + capitalize(input.camelName);
  //buffer.push(members.join('\n'));
  buffer = buffer.concat(members);
  for (index = 0; index < methods.length; index++) {
    var method = methods[index];
    ...
    if (method.args && method.args.length > 0) {
      argsList = 'Object ' + method.args.join(', Object ');
    }
    //method.name = camelize(method.name);
    ...
    if (method.name.indexOf("_") === 0) {
      buffer.push('private ' + method.returns + ' ' + method.name.split('_').join('') + '(' + argsList + '){' + returnString + '}');
    } else {
      buffer.push('public ' + method.returns + ' ' + method.name + '(' + argsList + '){' + returnString + '}');
    }
  }
  buffer.push('  public ' + capitalize(input.camelName) + '() {');
  buffer.push('  }');
  ...
  var originalPath = exportPath + '/' + packageSubpath + '/' + classFileName;
  //console.log('totalPackage: >>>>>>>>>>>>>>>>>>>>>>>>>>>>>> ' + totalPackage);
  tempPath = totalPackage.split('.');
  // chop off .java
  //tempPath.pop();
  tempPath = tempPath.join('/') + '.java';
  var virtualPath = exportPath + '/' + tempPath;
  writeFile(virtualPath, src);
  formatJava(virtualPath);
}
...
var generateTestProc = {
  id: 'generateTestProc',
  type: 'processor',
  description: 'Writes spec for each module.',
  process: function (input, doneCallback) {
    if (input.errors[this.id] == null) {
      input.errors[this.id] = [];
    }
    var amdProcData = input.results.amdProc;
    try {
      createTestSpec(input, amdProcData, input, function () {
        doneCallback(input);
      });
    } catch (exx) {
      console.error(exx.stack);
...
function writeFile(filePathName, source) {
  //console.log('writeFile', filePathName);
  if (WRITE_ENABLED) {
    filePathName = _path.normalize(filePathName);
...
function processSingleFile(options, completionCallback) {
  ...
  if (!projectRoot) {
    throw ('processSingleFile needs to know the project root');
  }
...
function processFile(modulePaths, baseDirectory, filePathName, outputDirectory,
  ...
  writeEnable, projectRoot) {
  FILE_IS_EMPTY = false;
  var output = {};
  output.results = {};
  output.errors = {};
  output.baseDirectory = baseDirectory;
  ...
  output.name = normalizeName(moduleName);
  output.camelName = camelize(output.name);
  output.relativePath = output.path.substring(output.baseDirectory.length);
  if (output.relativePath.charAt(0) === '/') {
    output.relativePath = output.relativePath.substring(1);
  }
  var source = readFile(filePathName);
  output.rawSource = source;
  output.source = source;
...
var modules = {};
/**
 * Old, old, old parse method that scrapes essential module data from a JS file.
 * @param   {string}   input        The source within a JS file.
 * @param   {string}   filePathname The actual file name.
 * @returns {object} Object full of metadata.
 */
    ...
    if (defineBlock && (!output.isMain)) {
      ...
      if (afterDefine.charAt(0) !== '{' && afterDefine.indexOf('[') !== -1) {
        ...
        if (afterDefineSplit.length > 1) {
          ...
          logger.log('afterDefine: ' + afterDefine);
          logger.log('Problem file: ' + filePathname);
        }
      }
...
var exportAMDData = {
  ...
  process: function (input, doneCallback) {
    ...
    depends.exports = input.className;
    for (var index = 0; index < amdProcData.requires.length; index++) {
      var module = amdProcData.requires[index];
      depends.requires[module] = 1;
    }
    // var modulePath = input.
    allModules[input.relativePath] = depends;
...
var stripCommentsProc = {
  ...
  process: function (input, doneCallback) {
    ...
    input.source = (stripJsDocComments(input.source));
    doneCallback(input);
  }
};
var moduleDataX = null;
var fixExportPathsProc = {
  id: 'fixExportPathsProc',
  type: 'processor',
  description: 'Ensure @exports tags have valid paths.',
  process: function (input, doneCallback) {
    if (moduleDataX === null){
      moduleDataX = JSON.parse(readFile('./modules.json'));
    }

    // moduleData has to be a cache of ALL modules already baked golden brown
    //  "editor-ui/core/component.js": {
    //    "requires": {
    //      "jquery": 1
    //    },
    //    "exports": "Component"
    //  },
    function findModule(className) {
      var moduleName = decamelize(className).split('_').join('-');
      for (var c in moduleDataX) {
        if (moduleDataX.hasOwnProperty(c)) {
          var module = moduleDataX[c];
          if (module.exports === className) {
            if (c.indexOf('/') !== -1) {
              moduleName = c.split('/').pop();
            } else {
              moduleName = c;
            }
    ...
    for (index = 0; index < lines.length; index++) {
      ...
      // if (line.indexOf('//') === -1 && line.indexOf('class ') !== -1 && line.indexOf(' extends ') !== -1 ){
      if (line.indexOf('//') === -1 && line.indexOf('@extends ') !== -1 && line.indexOf(' module:') === -1) {
        //logger.warn('EXTENDS >>> ', line);
        var baseClass = line.split('@extends')[1].trim();
        var moduleName = findModule(baseClass);
        var fullPath = 'module:' + moduleName + '~' + baseClass;
        line = line.split(' ' + baseClass).join(' ' + fullPath);
        //logger.warn('EXTENDS <<< ', line, baseClass, moduleName, fullPath);
        lines[index] = line;
      }
    }
    input.source = lines.join('\n');
    //console.log(input.processedFilePath);
    //writeFile(input.processedFilePath, input.source);
...
var plugins = {
  ...
  'generateTestProc': generateTestProc,
  ...
  'getES6ModuleDataProc': getES6ModuleDataProc,
  'fixExportPathsProc': fixExportPathsProc
};
/**
 * Get procs.
 *
