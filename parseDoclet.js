...
function fixDoclets(walkerObj) {
    ...
    if (ast.comments != null) {
        for (var index = 0; index < ast.comments.length; index++) {
            ...
            dCount++;
            // console.warn(doclet);
            // console.warn(printDoclet(doclet) + '\n');
            var newDoclet = printDoclet(doclet, defineModuleInTopOfFile);
            var formatted = newDoclet;
            // console.warn(newDoclet);
            ...
            // This all seems to build a JSON data structure we don't need for
            // anything.
            ...
            if (beforeRange < 0) {
                // console.warn('beforeRange < 0: ' + comment.value);
                beforeRange = 0;
            }
            ...
            // doclet needs to begin on its own line
            var lastChar = '\n';
            if (newFile.length > 0) {
                lastChar = newFile.split('').pop();
            }

            if (lastChar !== '\n' && lastChar !== ' ') {
                console
                        .warn('lastChar = "' + lastChar
                                + '", inserting newline');
                newFile += '\n';
            }
            ...
            // ?? does cursor get thrown off when we skip (continue;) above????
